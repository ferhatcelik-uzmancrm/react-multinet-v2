...
import { PhoneRequest } from "../../models/Phone";
...
export default function EmailsTable() {
  ...
  const handleArchiveClick = (companyId: string, companyType: string) => {
    ...
    navigate(`/emails/detail/${companyId}`);
  ...
  const handleApiSearch = async () => {
    try {
      ...
      if (searchQuery !== null && searchQuery !== "") {
        ...
        const response = await getCRMData('api/search-emails', searchParams);
        setEmails(response.data);
      } else {
        const response = await fetchUserData('api/get-emails', '');
        setEmails(response.data);
      }
    } catch (error) {
      alert(error);
  ...
  const filteredRows = emails !== null && Array.isArray(emails)
    ? emails.filter((row) =>
    ...
    <React.Fragment >
      ...
      >
        <FormControl sx={{ flex: 1 }} size="sm">
          <FormLabel>E-posta ara</FormLabel>
          <Input
            size="md"
            placeholder="Ara"
            startDecorator={<i data-feather="search" />}
        ...
        >
          <thead>
            <tr>
              ...
              <th style={{ width: 220, padding: 12 }}>Kimden</th>
              <th style={{ width: 220, padding: 12 }}>Kime</th>
              <th style={{ width: 220, padding: 12 }}>Bilgi</th>
              <th style={{ width: 120, padding: 12 }}>Gizli</th>
              <th style={{ width: 120, padding: 12 }}> </th>
            </tr>
          </thead>
          <tbody>
            {emails !== null && Array.isArray(emails) ? (
              stableSort(filteredRows, getComparator(order, "EmailId")).map((row) => (
                <tr key={row.EmailId}>
                  <td style={{ textAlign: "center" }}>
                    <Checkbox
                      checked={selected.includes(row.EmailId)}
                      color={selected.includes(row.EmailId) ? "primary" : undefined}
                      onChange={(event) => {
                        setSelected((ids) =>
                          event.target.checked
                            ? ids.concat(row.EmailId)
                            : ids.filter((itemId) => itemId !== row.EmailId)
                        );
                      }}
                  ...
                  <td>
                    {row.Bcc && row.Bcc.length > 0 ? (
                      row.Bcc.map((lookup, index) => (
                        <div key={index}>{lookup.Name}</div>
                      ))
                    ) : (
                      <span>No recipients</span>
                  ...
                  <td style={{ textAlign: "right" }}>
                    <Link
                      ...
                      component="button"
                      color="neutral"
                      onClick={() => handleArchiveClick(row.EmailId, row.Subject)}
                    >
                    ...
                    <Link
                      ...
                      color="primary"
                      sx={{ ml: 2 }}
                      onClick={() => handleExport([row], row.Subject, "xlsx")}
                    >
              ...
              <tr>
                <td colSpan={8} style={{ textAlign: "center" }}>
                  {emails === null ? "Loading..." : "No matching contacts found."}
                </td>
              </tr>
            )}
          </tbody>
