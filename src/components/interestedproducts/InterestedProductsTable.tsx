    ...
    const handleApiSearch = async () => {
        try {
            ...
            if (searchQuery !== null && searchQuery !== "") {
                ...
                const searchParams: InterestedProductRequest = {
                    ...interestedProductRequest,
                    Name: searchQuery
                };

                const response = await getCRMData('api/search-interestedproducts', searchParams);
                setInterestedproducts(response.data);
            } else {
                const response = await fetchUserData('api/get-interestedproducts', '');
                setInterestedproducts(response.data);
            }
        } catch (error) {
            alert(error);
                ...
                >
                    <thead>
                        <tr>
                            ...
                            <th style={{ width: 220, padding: 12 }}>Müşteri Adayı</th>
                        ...
                        {paginatedRows !== null && Array.isArray(paginatedRows) ? (
                            stableSort(paginatedRows, getComparator(order, "InterestedProductId")).map((row) => (
                                <tr key={row.InterestedProductId}>
                                    <td style={{ textAlign: "center" }}>
                                        <Checkbox
                                            checked={selected.includes(row.InterestedProductId)}
                                            color={selected.includes(row.InterestedProductId) ? "primary" : undefined}
                                            onChange={(event) => {
                                                setSelected((ids) =>
                                                    event.target.checked
                                                        ? ids.concat(row.InterestedProductId)
                                                        : ids.filter((itemId) => itemId !== row.InterestedProductId)
                                                );
                                            }}
                                    ...
                                    <td style={{ textAlign: "right" }}>
                                        <Link
                                            ...
                                            component="button"
                                            color="neutral"
                                            onClick={() => handleArchiveClick(row.InterestedProductId, row.Name)}
                                        >
                            ...
                            <tr>
                                <td colSpan={8} style={{ textAlign: "center" }}>
                                    {interestedproducts === null ? "Loading..." : "No matching contacts found."}
                                </td>
                            </tr>
                        )}
                    </tbody>
